impl Runtime {
    /// Validates a browser-supplied source in the same restricted Lua
    /// environment used by live block activation.
    pub fn validate_source(source: &str) -> Result<LogicRevision, LogicError> {
        Engine::validate_source(source)
    }
    ...
    /// Simulates a supplied draft source against copied state and timers. The
    /// active runtime and every signal/input projection remain untouched.
    pub fn simulate_input_with_source(
        ...
        let mut execution = block
            .engine
            .simulate_input_with_source(source, scenario, state, pending_timers, now)
            .map_err(|error| RuntimeSimulationError::Block {
                block_id: block_id.clone(),
                error,
            })?;
    ...
    /// Simulates a timer against a supplied draft source in isolation.
    pub fn simulate_timer_with_source(
        ...
        let mut execution = block
            .engine
            .simulate_timer_with_source(source, scenario)
            .map_err(|error| RuntimeSimulationError::Block {
                block_id: block_id.clone(),
                error,
            })?;
    ...
    /// Simulates a schedule occurrence against a supplied draft source. The
    /// schedule cursor and structural token are checked on the live runtime,
    /// while evaluation itself runs on an isolated engine clone.
    pub fn simulate_schedule_with_source(
        &self,
        request: ScheduleSimulationRequest,
        source: impl Into<String>,
    ) -> Result<BlockExecution, ScheduleSimulationError> {
        ...
        let mut execution = block
            .engine
            .simulate_schedule_trigger_with_source(
                source,
                trigger,
                &self.site,
                Some(request.occurrence_at_utc_ms),
            )
            .map_err(ScheduleSimulationError::InvalidSource)?;
        self.annotate_simulation(&request.block_id, &mut execution);
        Ok(BlockExecution {
            block_id: request.block_id,
            execution,
