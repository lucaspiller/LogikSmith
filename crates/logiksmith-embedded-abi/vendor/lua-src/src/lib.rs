...
impl Build {
    ...
    pub fn build(&mut self, version: Version) -> Artifacts {
        ...
        match target {
            ...
            // Lua's generic C implementation is suitable for the ESP32
            // newlib target. The cross compiler supplies the libc calls; no
            // POSIX/Windows platform shim is needed here.
            _ if target.starts_with("xtensa-") => {
                config
                    .flag("-mlongcalls")
