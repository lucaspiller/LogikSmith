import type {
  ...
  DisplayLogicError, DisplayLog, DisplayPendingTimer, DisplaySimulation, DisplaySnapshot, DisplayState,
  DisplayStateValue, DisplayTelegram, DisplayTimer, DisplayTimerEffect, DisplayTimerEffectAction,
...
export class SimulationApiError extends Error {
  ...
  constructor(status: number, message: string, fieldErrors: SimulationFieldError[] = []) { super(message); this.name = 'SimulationApiError'; this.status = status; this.fieldErrors = fieldErrors; }
}
...
function pendingTimer(value: unknown, index: number): DisplayPendingTimer {
  const path = `logic.pending_timers[${index}]`; const source = object(value, path);
  return { name: stringValue(required(field(source, path, 'name', 'timer'), `${path}.name`), `${path}.name`), scheduledAtMs: integer(required(field(source, path, 'scheduled_at_ms', 'scheduledAtMs'), `${path}.scheduled_at_ms`), `${path}.scheduled_at_ms`), dueAtMs: integer(required(field(source, path, 'due_at_ms', 'dueAtMs'), `${path}.due_at_ms`), `${path}.due_at_ms`), logicRevision: revision(required(field(source, path, 'logic_revision', 'logicRevision'), `${path}.logic_revision`), `${path}.logic_revision`) };
}
...
function write(value: unknown): DisplayWrite {
  ...
  const requestRaw = field(record, 'write', 'request_id', 'requestId'); return { status, requestId: requestRaw === undefined || requestRaw === null ? null : revision(requestRaw, 'write.request_id'), value: displayValue(field(record, 'write', 'value'), 'write.value'), error: optionalString(field(record, 'write', 'error'), 'write.error') };
}
...
function timerTrigger(source: JsonObject, path: string): DisplayTimerExecutionTrigger {
  ...
  return { type: 'timer', name, timer: name, scheduledAtMs, dueAtMs, firedAtMs, lateByMs: lateRaw === undefined || lateRaw === null ? Math.max(0, firedAtMs - dueAtMs) : nonNegativeNumber(lateRaw, `${path}.late_by_ms`), scheduledLogicRevision: revision(required(field(source, path, 'scheduled_logic_revision', 'scheduledLogicRevision', 'logic_revision'), `${path}.scheduled_logic_revision`), `${path}.scheduled_logic_revision`) };
}
function executionTrigger(value: unknown, path: string): DisplayExecutionTrigger {
  const source = object(value, path); const type = field(source, path, 'type'); if (type === 'timer' || field(source, path, 'name', 'timer') !== undefined && field(source, path, 'fired_at_ms', 'firedAtMs', 'fired_at') !== undefined) return timerTrigger(source, path); return inputTrigger(source, path);
}
...
function execution(value: unknown, index: number): DisplayExecution {
  const path = `logic.executions[${index}]`; const source = object(value, path); const status = stringValue(required(field(source, path, 'status'), `${path}.status`), `${path}.status`); if (status !== 'succeeded' && status !== 'failed') throw new ApiDecodeError(`${path}.status`, `unsupported status ${status}`);
  ...
  const before = stateMap(field(source, path, 'state_before', 'stateBefore'), `${path}.state_before`); const after = stateMap(field(source, path, 'state_after', 'stateAfter'), `${path}.state_after`); return { executionId: integer(required(field(source, path, 'execution_id', 'executionId'), `${path}.execution_id`), `${path}.execution_id`), timeMs: integer(required(field(source, path, 'time_ms', 'timeMs'), `${path}.time_ms`), `${path}.time_ms`), durationUs: integer(required(field(source, path, 'duration_us', 'durationUs'), `${path}.duration_us`), `${path}.duration_us`), logicRevision: optionalRevision(required(field(source, path, 'logic_revision', 'logicRevision'), `${path}.logic_revision`), `${path}.logic_revision`), status: status as DisplayExecution['status'], trigger: executionTrigger(required(field(source, path, 'trigger'), `${path}.trigger`), `${path}.trigger`), inputs, transition: parsedTransition, stateBefore: before, stateAfter: after, effects, timerEffects, error: field(source, path, 'error') === undefined || field(source, path, 'error') === null ? null : logicError(field(source, path, 'error'), `${path}.error`) };
}
...
export function decodeSnapshot(input: unknown, receivedAtMs = Date.now()): DisplaySnapshot {
  ...
  const activeStructuralRevision = optionalRevision(field(root, 'snapshot', 'active_structural_revision', 'activeStructuralRevision') ?? readLogic(['active_structural_revision', 'activeStructuralRevision']) ?? field(config, 'config', 'active_structural_revision', 'activeStructuralRevision'), 'active_structural_revision');
  const savedStructuralRevision = optionalRevision(field(root, 'snapshot', 'saved_structural_revision', 'savedStructuralRevision') ?? readLogic(['saved_structural_revision', 'savedStructuralRevision']) ?? field(config, 'config', 'saved_structural_revision', 'savedStructuralRevision'), 'saved_structural_revision');
  const activeLogicRevision = optionalRevision(field(root, 'snapshot', 'active_logic_revision', 'activeLogicRevision') ?? readLogic(['active_logic_revision', 'activeLogicRevision']), 'active_logic_revision');
  const savedLogicRevision = optionalRevision(field(root, 'snapshot', 'saved_logic_revision', 'savedLogicRevision') ?? readLogic(['saved_logic_revision', 'savedLogicRevision']), 'saved_logic_revision');
  const executions = logicStatus ? array(required(field(logicStatus, 'logic', 'executions'), 'logic.executions'), 'logic.executions').map(execution) : [];
  ...
  const pendingTimers = pendingRaw === undefined || pendingRaw === null ? [] : array(pendingRaw, 'pending_timers').map(pendingTimer);
  ...
  const restartRequired = explicitRestart === true || (hasStructuralRevisions ? activeStructuralRevision !== savedStructuralRevision : activeAutomationRevision !== null && savedAutomationRevision !== null && activeAutomationRevision !== savedAutomationRevision);
  ...
  return {
    ...
    restartRequired, capturedAtMs, clockOffsetMs, state: stateMap(stateRaw, 'state'), pendingTimers, executions,
    write: write(field(root, 'snapshot', 'write') ?? field(root, 'write', 'write_status', 'last_write')),
    timer: legacyTimer, telegrams: telegrams as DisplayTelegram[], logs: logs as DisplayLog[]
  };
}
...
export async function simulateScenario(scenario: SimulationScenario, fetchImpl: FetchLike = fetch): Promise<DisplaySimulation> {
  ...
  const body: Record<string, unknown> = { expected_logic_revision: scenario.expectedLogicRevision, trigger, inputs: scenario.inputs.map((input) => ({ endpoint: input.endpoint, value: input.value, valid: input.valid, age_ms: input.ageMs })) };
  if (scenario.state !== undefined) body.state = scenario.state;
  if (scenario.pendingTimers !== undefined) body.pending_timers = scenario.pendingTimers.map((timer) => ({ name: timer.name, scheduled_at_ms: timer.scheduledAtMs, due_at_ms: timer.dueAtMs, logic_revision: timer.logicRevision }));
  ...
  if (response.ok) return decodeSimulation(await response.json()); const errorBody = await jsonOrNull(response); throw new SimulationApiError(response.status, simulationErrorMessage(response.status, errorBody), simulationFieldErrors(errorBody));
}
