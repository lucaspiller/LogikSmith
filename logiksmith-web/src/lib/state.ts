...
export interface DisplayTimerExecutionTrigger { type: 'timer'; name: string; timer: string; scheduledAtMs: number; dueAtMs: number; firedAtMs: number; lateByMs: number; scheduledLogicRevision: number; }
...
export interface DisplayPendingTimer { name: string; scheduledAtMs: number; dueAtMs: number; logicRevision: number; }
...
export interface SimulationScenario { expectedLogicRevision: number; trigger: SimulationTriggerRequest; inputs: SimulationInputRequest[]; state?: DisplayState; pendingTimers?: DisplayPendingTimer[]; }
...
export interface DisplayWrite { status: WriteStatus; requestId: number | null; value: boolean | number | null; error: string | null; }
...
export interface DashboardState { snapshot: DisplaySnapshot | null; revision: number; streamStatus: StreamStatus; stale: boolean; staleAtMs: number | null; error: string | null; needsResync: boolean; nowMs: number; selectedExecutionId: number | null; selectionPinned: boolean; selectionNotice: string | null; }
...
export const initialDashboardState: DashboardState = { snapshot: null, revision: 0, streamStatus: 'connecting', stale: true, staleAtMs: null, error: null, needsResync: false, nowMs: 0, selectedExecutionId: null, selectionPinned: false, selectionNotice: null };
...
function reconcileSelection(state: DashboardState, snapshot: DisplaySnapshot): Pick<DashboardState, 'selectedExecutionId' | 'selectionPinned' | 'selectionNotice'> {
  ...
  if (state.selectedExecutionId !== null && snapshot.executions.some((execution) => execution.executionId === state.selectedExecutionId)) return { selectedExecutionId: state.selectedExecutionId, selectionPinned: true, selectionNotice: null };
...
export function reduceDashboardState(state: DashboardState, action: DashboardAction): DashboardState {
  switch (action.type) {
    case 'snapshot_loaded': { const stale = state.snapshot !== null && state.stale; return { ...state, snapshot: action.snapshot, revision: action.snapshot.revision, streamStatus: 'connecting', stale, staleAtMs: stale ? state.staleAtMs : null, error: null, needsResync: false, nowMs: action.nowMs ?? state.nowMs, ...reconcileSelection(state, action.snapshot) }; }
    ...
    case 'event_received': {
      ...
      return { ...state, snapshot: event.snapshot, revision: event.revision, error: null, needsResync: false, streamStatus: 'connected', stale: false, staleAtMs: null, ...reconcileSelection(state, event.snapshot) };
    }
    case 'select_execution': {
      if (!state.snapshot) return state;
      const execution = state.snapshot.executions.find((item) => item.executionId === action.executionId);
      if (!execution) return state;
      return { ...state, selectedExecutionId: execution.executionId, selectionPinned: execution.executionId !== state.snapshot.executions[0]?.executionId, selectionNotice: null };
    }
